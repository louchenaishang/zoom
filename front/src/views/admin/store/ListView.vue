  ...
  <section>
    ...
    <template>
      <el-table :data="list" highlight-current-row v-loading="listLoading" :stripe="true" style="width: 100%;">
        ...
        <el-table-column prop="address" label="店铺地址" sortable>
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        Api.getStoreList(para).then((res) => {
      ...
      handleEdit: function (row) {
        this.$router.push({path: '/admin/store/form?type=edit&id='+row.id})
      },
      //显示新增界面
      handleAdd: function () {
        this.$router.push({path: '/admin/store/form?type=add'})
      },
    },
    mounted() {
      this.getList()
